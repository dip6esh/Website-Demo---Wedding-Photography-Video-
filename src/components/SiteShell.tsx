    ...
    <div className="grain min-h-screen bg-background text-foreground pb-16 md:pb-0">
      <header className="sticky top-0 z-40 border-b border-foreground/15 bg-background/95 backdrop-blur-sm">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <nav className="hidden items-center gap-8 font-mono text-[11px] uppercase tracking-[0.18em] md:flex" aria-label="Primary navigation">
            {navItems.map((item) => (
              <Link key={item.to} to={item.to} preload="intent" activeProps={{ className: "text-foreground" }} className="text-foreground/55 transition-colors hover:text-primary">
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <Link to="/contact" preload="intent" className="hidden rounded-full bg-foreground px-4 py-2 text-sm font-medium text-background transition-transform hover:-translate-y-0.5 active:translate-y-0 sm:inline-flex">
              Enquire
            </Link>
        ...
        {menuOpen && (
          <nav className="border-t border-foreground/15 px-5 py-3 md:hidden" aria-label="Mobile navigation">
            {navItems.map((item) => (
              <Link key={item.to} to={item.to} preload="intent" onClick={() => setMenuOpen(false)} className="flex items-center justify-between border-b border-foreground/10 py-4 font-mono text-xs uppercase tracking-[0.18em] last:border-0">
                {item.label}
      ...
      <footer className="border-t border-foreground/15 bg-foreground text-background">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-10 md:grid-cols-[1.2fr_1fr_1fr]">
          ...
          <div className="font-mono text-[11px] uppercase leading-loose tracking-[0.15em] text-background/60">
            ...
            {navItems.map((item) => <Link key={item.to} to={item.to} preload="intent" className="block transition-colors hover:text-background">{item.label}</Link>)}
          </div>
        </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-foreground/15 bg-background/95 md:hidden">
        <div className="flex items-stretch">
          <Link to="/works" preload="intent" className="flex flex-1 flex-col items-center justify-center py-3 text-foreground/65 active:bg-card"><span className="mb-1 text-sm">◌</span><span className="font-mono text-[10px] uppercase tracking-[0.14em]">Works</span></Link>
          <Link to="/services" preload="intent" className="flex flex-1 flex-col items-center justify-center py-3 text-foreground/65 active:bg-card"><span className="mb-1 text-sm">＋</span><span className="font-mono text-[10px] uppercase tracking-[0.14em]">Services</span></Link>
