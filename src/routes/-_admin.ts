...
import { invalidatePublicAlbumsCache } from "./-_albums.list";
import {
  AdminSession,
...
// createUploadTokenFn — issues a short-lived signed upload URL so the browser
// can PUT the file directly to Supabase Storage, bypassing the server for the
// binary payload and giving real XHR upload-progress events.
const UploadTokenSchema = z.object({
  filename: z.string().min(1).max(200),
  contentType: z.string().min(3).max(100),
});

export const createUploadTokenFn = createServerFn({ method: "POST" })
  .validator((body: unknown) => UploadTokenSchema.safeParse(body))
  .handler(async (args) => {
    debugAuth("createUploadTokenFn", args);
    const authed = await isAdminFromCtx(args);
    if (!authed) return await unauth("createUploadTokenFn", args);
    const parse = (args?.data ?? undefined) as ReturnType<typeof UploadTokenSchema.safeParse> | undefined;
    if (!parse || !parse.success) {
      return { ok: false as const, code: "VALIDATION" as const, message: "Bad request." };
    }
    if (!ACCEPTED.includes(parse.data.contentType)) {
      return { ok: false as const, code: "VALIDATION" as const, message: "Unsupported image type (JPG, PNG, WebP, GIF, AVIF only.)" };
    }
    const res = await createSignedUploadUrl(parse.data);
    if ("error" in res) return { ok: false as const, code: "SERVER" as const, message: res.error };
    return { ok: true as const, signedUrl: res.signedUrl, path: res.path, publicUrl: res.publicUrl };
  });
...
// ---------------- Album Videos CRUD (Admin) ----------------

const AlbumVideoSchema = z.object({
  album_id: z.string().min(1).max(64),
  youtube_url: z.string().trim().min(1).max(500),
  title: z.string().trim().max(200).default(""),
});

const AlbumVideoIdSchema = z.object({
  id: z.string().min(1).max(64),
});

const SetFeaturedVideoSchema = z.object({
  album_id: z.string().min(1).max(64),
  video_id: z.string().min(1).max(64).nullable(),
});

export const listAlbumVideosAdmin = createServerFn({ method: "POST" })
  .validator((body: unknown) => AlbumIdSchema.safeParse(body))
  .handler(async (args) => {
    debugAuth("listAlbumVideosAdmin", args);
    const authed = await isAdminFromCtx(args);
    if (!authed) return await unauth("listAlbumVideosAdmin", args);
    const parse = (args?.data ?? undefined) as ReturnType<typeof AlbumIdSchema.safeParse> | undefined;
    if (!parse || !parse.success) return { ok: false as const, code: "VALIDATION" as const, message: "Missing album_id." };
    const r = await listAlbumVideos(parse.data.album_id);
    if ("error" in r) return { ok: false as const, code: "SERVER" as const, message: r.error };
    return { ok: true as const, videos: r.videos as AlbumVideo[] };
  });

export const addAlbumVideoAdmin = createServerFn({ method: "POST" })
  .validator((body: unknown) => AlbumVideoSchema.safeParse(body))
  .handler(async (args) => {
    debugAuth("addAlbumVideoAdmin", args);
    const authed = await isAdminFromCtx(args);
    if (!authed) return await unauth("addAlbumVideoAdmin", args);
    const parse = (args?.data ?? undefined) as ReturnType<typeof AlbumVideoSchema.safeParse> | undefined;
    if (!parse || !parse.success) return { ok: false as const, code: "VALIDATION" as const, message: "Invalid video payload." };
    // Count existing videos to set sort_order
    const existing = await listAlbumVideos(parse.data.album_id);
    const nextSort = "videos" in existing && existing.videos.length
      ? Math.max(...existing.videos.map((v) => v.sort_order ?? 0)) + 1
      : 0;
    const res = await insertAlbumVideo({
      album_id: parse.data.album_id,
      youtube_url: parse.data.youtube_url,
...
export const deleteAlbumVideoAdmin = createServerFn({ method: "POST" })
  .validator((body: unknown) => AlbumVideoIdSchema.safeParse(body))
  .handler(async (args) => {
    debugAuth("deleteAlbumVideoAdmin", args);
    const authed = await isAdminFromCtx(args);
    if (!authed) return await unauth("deleteAlbumVideoAdmin", args);
    const parse = (args?.data ?? undefined) as ReturnType<typeof AlbumVideoIdSchema.safeParse> | undefined;
    if (!parse || !parse.success) return { ok: false as const, code: "VALIDATION" as const, message: "Missing video id." };
    const res = await deleteAlbumVideo(parse.data.id);
    if ("error" in res) return { ok: false as const, code: "SERVER" as const, message: res.error };
    invalidatePublicAlbumsCache();
    return { ok: true as const };
  });

export const setFeaturedAlbumVideoAdmin = createServerFn({ method: "POST" })
  .validator((body: unknown) => SetFeaturedVideoSchema.safeParse(body))
  .handler(async (args) => {
    debugAuth("setFeaturedAlbumVideoAdmin", args);
    const authed = await isAdminFromCtx(args);
    if (!authed) return await unauth("setFeaturedAlbumVideoAdmin", args);
    const parse = (args?.data ?? undefined) as ReturnType<typeof SetFeaturedVideoSchema.safeParse> | undefined;
    if (!parse || !parse.success) return { ok: false as const, code: "VALIDATION" as const, message: "Invalid payload." };
    const res = await setFeaturedAlbumVideo(parse.data.album_id, parse.data.video_id);
    if ("error" in res) return { ok: false as const, code: "SERVER" as const, message: res.error };
    invalidatePublicAlbumsCache();
    return { ok: true as const };
  });

const HeroTileUpdateSchema = z.object({
...
export const updateHeroTileAdmin = createServerFn({ method: "POST" })
  .validator((body: unknown) => HeroTileUpdateSchema.safeParse(body))
  .handler(async (args) => {
    debugAuth("updateHeroTileAdmin", args);
    const authed = await isAdminFromCtx(args);
    if (!authed) return await unauth("updateHeroTileAdmin", args);
    const parse = (args?.data ?? undefined) as ReturnType<typeof HeroTileUpdateSchema.safeParse> | undefined;
    if (!parse || !parse.success) return { ok: false as const, code: "VALIDATION" as const, message: "Invalid hero tile payload." };
    const { slot, ...fields } = parse.data;
    const res = await updateHeroTile(slot, fields);
    if ("error" in res) return { ok: false as const, code: "SERVER" as const, message: res.error };
    return { ok: true as const };
  });
