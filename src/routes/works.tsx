...
function WorksPage() {
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 md:py-20"><div className="mb-8 flex flex-wrap items-center gap-2" role="tablist" aria-label="Portfolio categories">{categories.map((item) => <button key={item} type="button" role="tab" aria-selected={category === item} onClick={() => setCategory(item)} className={`rounded-full px-4 py-2 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors ${category === item ? "bg-foreground text-background" : "ring-1 ring-foreground/20 hover:bg-card"}`}>{item}</button>)}</div><div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">{filteredPortfolio.map((item) => <article key={item.id} className="flex flex-col overflow-hidden rounded-xl bg-card ring-1 ring-foreground/10 transition-shadow hover:shadow-lg hover:ring-foreground/20"><div className="relative aspect-[4/5] overflow-hidden"><img src={item.image} alt={item.alt} width={1024} height={1280} loading="lazy" className="size-full object-cover transition-transform duration-700 group-hover:scale-105 hover:scale-105" /></div><div className="flex flex-1 flex-col justify-between px-5 py-4"><div><p className="font-display text-lg font-semibold leading-tight">{item.title}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-foreground/55">{item.location}</p></div><p className="mt-3 inline-flex items-center gap-1 rounded-full bg-primary/10 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.14em] text-primary/90">{item.category}</p></div></article>)}</div></section>
